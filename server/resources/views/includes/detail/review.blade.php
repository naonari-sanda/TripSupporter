...
<div class="wrapper">
    ...
    <div class="fill mb-3">
        ...
        <p class="text mb-0">{!! nl2br(e($review->review)) !!}</p>
    </div>

    @if(!empty($review->imgpath))
    <a class="d-block" href="{{ $review->imgpath }}" data-lightbox="review-{{ $review->id }}" title="<p class='title'>{{ $review->user->name }}さんの{{ $review->country->name  }}の思い出。</p><p class='text'>{{ $review->updated_at->format('Y年m月d日') }}に投稿">
        <img class="img img-thumbnail" src="{{ $review->imgpath }}" style="width: 200px;" />
    </a>
    @endif
