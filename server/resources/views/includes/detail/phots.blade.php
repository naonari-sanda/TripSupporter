...
<div id="lightgallery ">
    <div class="row">
        ...
        <div class="bg col-md-3 col-6 mb-2">
            <a class="d-block" href="{{ asset('/storage/' . $review->imgpath ) }}" data-lightbox="example-1" title="<p class='title'>{{ $review->user->name }}さんの{{ $review->country->name  }}の思い出。</p><p class='text'>{{ $review->review }}</p><p><p>{{ $review->updated_at->format('Y年m月d日') }}に投稿</p>">
        ...
        <div class="bg col-md-3 col-6 mb-2">
            <a class="d-block" href="{{ asset('/storage/' . $img->imgpath ) }}" data-lightbox="example-1" title="<p class='title'>{{ $img->user->name }}さんの{{ $img->country->name  }}の思い出。</p><p class='text'>{{ $img->updated_at->format('Y年m月d日') }}に投稿">
